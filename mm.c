...
static int nearest_two(int n){               /*nearest two function returns the nearest ceil power of 2*/
  int num = __builtin_clz(n);
  num =  31-num;
  if ( (1<<num) ==n ){ return num; }
  else{ return num+1; }
}



static void *find_root(size_t size){          /*Fetch and return the root according to the requested size*/
 if(size < BOUNDARY){
...
static void set_root(size_t size, void *ptr){      /*Fet the root the the given address*/ 
  ...
  if(size < BOUNDARY){ 
    ...
    degree = nearest_two(size);
    if(degree > 18){PUTP(root_last,ptr);}
    else{PUTP(heap_start+degree*WSIZE,ptr);}
  }
}

static void* sparse_fit(size_t asize){           /*Find the fitting free list within the sparse free list class */
  void *bp;
...
static void* dense_fit(size_t asize){         /*Find the fitting free list within the dense FERE LIST class */
  void *bp;
  start = find_root(asize);
...
static void disconnect(void* bp){            /*Disconnect the given FREE BLOCK   !!!CHECK FOR THE EDGE CASES!!!   */
...
static void make_root(void* bp){            /*Use LIFO policy to insert the FREE BLOCK at the start of the appropriate free list class */
...
void *mm_malloc(size_t size) {
  ...
  if(asize <24){ asize=24; } /*the least allocation size to accomodate explicit free list feature */

  /* Search the free list for a fit */

  if(asize <=128){bp = dense_fit(asize);}
  else{bp = sparse_fit(asize);}
